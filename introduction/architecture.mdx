...
Let's walk through how Asymptote evaluates code as it's generated in [Claude Code](/integrations/claude-code) and [Cursor](/integrations/cursor), then flags [vulnerabilities](/introduction/core-concepts#vulnerability) before the change reaches a pull request.

<img src="/public/system_architecture.png" alt="Asymptote System Architecture" />

1. **Codegen-time invocation:** As your [AI coding agents](/introduction/core-concepts#coding-agent) draft or edit code, Asymptote is invoked through a mix of rules and [hooks](/introduction/core-concepts#hooks) in Claude Code and Cursor. This means analysis happens while code is being produced, not after the PR is opened.

2. **Request creation:** The hook invokes the Asymptote server with the proposed change, including the generated diff, files touched, repository metadata, and the active security policy profile.
...
7. **Downstream enforcement:** The same [policies](/introduction/core-concepts#policy) and detectors can run again in CI when a PR is opened, ensuring consistency between what was allowed during code generation and what is allowed to merge.
